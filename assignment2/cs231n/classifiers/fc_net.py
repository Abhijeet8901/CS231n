...
class TwoLayerNet(object):
    ...
    def __init__(
        ...
        self.params['W1']=weight_scale*np.random.randn(np.product(input_dim),hidden_dim);
    ...
    def loss(self, X, y=None):
        ...
        dlayer2,dW2,db2=affine_backward(dscores,cache2)
        dx,dW1,db1=affine_relu_backward(dlayer2,cache1)
        dW1+=reg*W1
        dW2+=reg*W2
        grads['W1']=dW1
        grads['W2']=dW2
...
class FullyConnectedNet(object):
    ...
    def loss(self, X, y=None):
        ...
        for i in range(1,self.num_layers):
            ...
            out,cache=affine_norm_relu_forward(out,gamma,beta,w,b,bn_params,self.normalization,self.use_dropout,self.dropout_param)
            caches.append(cache)
        ...
        for i in reversed(range(1,self.num_layers)):
            dout,dw,db,dgamma,dbeta=affine_norm_relu_backward(dout,caches[i-1],self.use_dropout,self.normalization)
...
def affine_norm_relu_forward(x,gamma,beta,w,b,bn_param,norm,drop,drop_param):
    ...
    return out,(aff_cache,norm_cache,relu_cache,drop_cache)


def affine_norm_relu_backward(dout,cache,drop,norm):

    aff_cache,norm_cache,relu_cache,drop_cache= cache
