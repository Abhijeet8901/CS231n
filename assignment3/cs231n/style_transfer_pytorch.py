...
def gram_matrix(features, normalize=True):
    ...
    f1=features.reshape(N*C,-1)
    f2=features.reshape(N*C,-1).transpose(1,0)
    gram=torch.mm(f1,f2)
...
def style_loss(feats, style_layers, style_targets, style_weights):
    ...
    for i,l in enumerate(style_layers):
      loss+=style_weights[i]*torch.sum((gram_matrix(feats[l])-style_targets[i])**2)
    pass
    return loss
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
...
def tv_loss(img, tv_weight):
    ...
    img1=img[:,:,1:,:]
    img2=img[:,:,:-1,:]
    loss=tv_weight*(torch.sum((img2-img1)**2))
    img1=img[:,:,:,1:]
    img2=img[:,:,:,:-1]
    loss+=tv_weight*(torch.sum((img2-img1)**2))
    pass
